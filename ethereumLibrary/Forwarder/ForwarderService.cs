...
using ethereumLibrary.Forwarder.ContractDefinition;
using System.Numerics;

namespace ethereumLibrary.Forwarder
{
    public partial class ForwarderService
    {
        ...
        public Task<string> FlushTokensRequestAsync(string tokenContractAddress, BigInteger? gas = null, BigInteger? gasPrice = null, BigInteger? nonce = null)
        {
            var flushTokensFunction = new FlushTokensFunction();
            flushTokensFunction.TokenContractAddress = tokenContractAddress;
            flushTokensFunction.Gas = gas;
            flushTokensFunction.GasPrice = gasPrice;
            flushTokensFunction.Nonce = nonce;
        ...
        public Task<TransactionReceipt> FlushTokensRequestAndWaitForReceiptAsync(string tokenContractAddress, CancellationTokenSource cancellationToken = null)
        {
            var flushTokensFunction = new FlushTokensFunction();
            flushTokensFunction.TokenContractAddress = tokenContractAddress;
            //flushTokensFunction.GasPrice = new HexBigInteger(140000000000);

            return ContractHandler.SendRequestAndWaitForReceiptAsync(flushTokensFunction, cancellationToken);
        }

        public TransactionReceipt FlushTokensRequestAndWaitForReceipt(string tokenContractAddress, CancellationTokenSource cancellationToken = null)
        {
            var flushTokensFunction = new FlushTokensFunction();
            flushTokensFunction.TokenContractAddress = tokenContractAddress;
            //flushTokensFunction.GasPrice = new HexBigInteger(240000000000);

            return ContractHandler.SendRequestAndWaitForReceiptAsync(flushTokensFunction, cancellationToken).Result;
        }
